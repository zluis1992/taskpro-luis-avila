  ...
  return (
    <div className="taskpro-app">
      ...
      {mounted && (
        <>
          {isMobile && menuOpened && (
            <div className="taskpro-overlay" onClick={() => setMenuOpened(false)} />
          )}
          <div
              ...
              <TreeView
                ...
                itemRender={(item: NavItem) => (
                  <div className="taskpro-nav-item">
                    <i className={`dx-icon dx-icon-${item.icon ?? 'bulletlist'}`} />
                    {menuOpened && <span className="taskpro-nav-label">{item.text}</span>}
                  </div>
                )}
              />
            </div>
      ...
      <div
        ...
        style={!mounted || (isMobile && menuOpened) ? undefined : { marginLeft: menuOpened ? 260 : 56 }}
      >
        <div className="taskpro-main">{children}</div>
        <footer className="taskpro-footer">
          <span>© 2025 TaskPro. Todos los derechos reservados.</span>
